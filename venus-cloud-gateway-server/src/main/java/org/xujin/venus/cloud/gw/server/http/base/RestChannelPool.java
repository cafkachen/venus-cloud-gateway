 ...
 */
public final class RestChannelPool extends RestSimpleChannelPool {
	...
	// There is no need to worry about synchronization as everything that modified the
	// queue or counts is done
	// by the above EventExecutor.
	private final Queue<AcquireTask> pendingAcquireQueue = new ArrayDeque<AcquireTask>();
	private final int maxConnections;
	private final int maxPendingAcquires;
	private volatile int acquiredChannelCount;
	 ...
	 * @param maxConnections the numnber of maximal active connections, once this is
	 * reached new tries to acquire a {@link Channel} will be delayed until a connection
	 * is returned to the pool again.
	 */
	public RestChannelPool(Bootstrap bootstrap, ChannelPoolHandler handler,
			int maxConnections) {
	 ...
	 * reached new tries to acquire a {@link Channel} will be delayed until a connection
	 * is returned to the pool again.
	 * @param maxPendingAcquires the maximum number of pending acquires. Once this is
	 * exceed acquire tries will be failed.
	 */
	public RestChannelPool(Bootstrap bootstrap, ChannelPoolHandler handler,
			int maxConnections, int maxPendingAcquires) {
		this(bootstrap, handler, ChannelHealthChecker.ACTIVE, null, -1, maxConnections,
				maxPendingAcquires);
	}
	 ...
	 * {@link Channel} is still healty when obtain from the {@link ChannelPool}
	 * @param action the {@link AcquireTimeoutAction} to use or {@code null} if non should
	 * be used. In this case {@param acquireTimeoutMillis} must be {@code -1}.
	 * @param acquireTimeoutMillis the time (in milliseconds) after which an pending
	 * acquire must complete or the {@link AcquireTimeoutAction} takes place.
	 * @param maxConnections the numnber of maximal active connections, once this is
	 * reached new tries to acquire a {@link Channel} will be delayed until a connection
	 * is returned to the pool again.
	 * @param maxPendingAcquires the maximum number of pending acquires. Once this is
	...
	public RestChannelPool(Bootstrap bootstrap, ChannelPoolHandler handler,
			...
			final long acquireTimeoutMillis, int maxConnections, int maxPendingAcquires) {
	 ...
	 * @param maxPendingAcquires the maximum number of pending acquires. Once this is
	 * exceed acquire tries will be failed.
	 * @param releaseHealthCheck will check channel health before offering back if this
	...
	public RestChannelPool(Bootstrap bootstrap, ChannelPoolHandler handler,
		...
		if (maxConnections < 1) {
			throw new IllegalArgumentException(
					"maxConnections: " + maxConnections + " (expected: >= 1)");
		}
		if (maxPendingAcquires < 1) {
			throw new IllegalArgumentException(
					"maxPendingAcquires: " + maxPendingAcquires + " (expected: >= 1)");
		}
		if (action == null && acquireTimeoutMillis == -1) {
			...
			throw new IllegalArgumentException("acquireTimeoutMillis: "
					+ acquireTimeoutMillis + " (expected: >= 1)");
		}
		else {
			...
			case NEW:
				timeoutTask = new TimeoutTask() {
					@Override
					public void onTimeout(AcquireTask task) {
						// Increment the acquire count and delegate to super to actually
						// acquire a Channel which will
						// create a new connetion.
						task.acquired();

						RestChannelPool.super.acquire(task.promise);
					}
				};
				break;
			default:
	...
	private void acquire0(final Promise<Channel> promise) {
		...
		if (acquiredChannelCount < maxConnections) {
			...
			// We need to create a new promise as we need to ensure the AcquireListener
			// runs in the correct
			// EventLoop
			...
			if (pendingAcquireCount >= maxPendingAcquires) {
				...
				if (pendingAcquireQueue.offer(task)) {
					...
					if (timeoutTask != null) {
						task.timeoutFuture = executor.schedule(timeoutTask,
								acquireTimeoutNanos, TimeUnit.NANOSECONDS);
					}
				}
				else {
					promise.setFailure(FULL_EXCEPTION);
	...
	public Future<Void> forceClose(final Channel channel, final Promise<Void> promise) {
		...
		super.forceClose(channel, p.addListener(new FutureListener<Void>() {
			@Override
		...
		super.release(channel, p.addListener(new FutureListener<Void>() {
			...
			public void operationComplete(Future<Void> future) throws Exception {
				...
				if (future.isSuccess()) {
					...
					// Check if the exception was not because of we passed the Channel to
					// the wrong pool.
	...
	private void decrementAndRunTaskQueue() {
		...
		// Run the pending acquire tasks before notify the original promise so if the user
		// would
		// try to acquire again from the ChannelFutureListener and the pendingAcquireCount
		// is >=
		// maxPendingAcquires we may be able to run some pending tasks first and so allow
		// to add
		// more.
		runTaskQueue();
	}
